...
for (const ed of editions) {
  ...
  test(`[${ed}] #9 the base-view caveat is in the files that need it, and nowhere else`, async (t) => {
    // The promise: an agent that greps this corpus, gets a hit and quotes it is TOLD when the text
    // it just quoted was kept from a container the NCC 2025 draft marked as new — because the
    // sub-numbering there is the draft's and the wording is the draft author's re-typing. Before
    // this, the only place that was said was the build report, which is stdout and ships nowhere:
    // the disclosure chain terminated in a document the consumer cannot reach.
    //
    // Checked as an EQUALITY against `{edition}/INDEX.md`, which is generated by the same build
    // from what it emitted. That makes it load-bearing in both directions: stop writing the notes
    // and the index still lists the files, so this goes red; spray the token over every file to
    // make it pass and the index does not list them, so it goes red too.
    const TOKEN = 'BASE-VIEW RETENTION:';   // literal, not imported: it is a promise to consumers
    // Edition-relative and `/`-separated, which is how the index writes a path on every platform.
    const relOf = f => f.split(path.sep).join('/').slice(`corpus/${ed}/`.length);
    const carrying = files(ed).filter(f => read(f).includes(TOKEN)).map(relOf).sort(byCodepoint);

    // Always valid, on any slice: a note may only sit where a note is placed, one line under the
    // H1, and it must be ONE line — a wrapped note breaks the phrase grep it exists to protect.
    for (const f of carrying) {
      const lines = read(path.join('corpus', ed, ...f.split('/'))).split('\n');
      const at = lines.findIndex(l => l.startsWith(TOKEN));
      assert.ok(at > 0 && lines[at - 1] === '' && lines[at + 1] === '',
        `${ed}/${f}: the caveat must be its own paragraph, blank line either side`);
      assert.ok(lines.slice(0, at).some(l => l.startsWith('# ')),
        `${ed}/${f}: the caveat must follow the H1 — the reader meets it before the text it qualifies`);
      assert.equal(lines.filter(l => l.startsWith(TOKEN)).length, 1, `${ed}/${f}: one caveat per file`);
    }

    const index = fs.existsSync(`corpus/${ed}/INDEX.md`) ? read(`corpus/${ed}/INDEX.md`) : '';
    const listed = [...index.matchAll(/^ {2}(\S+\.md) — \d+ retention sites?$/gm)].map(m => m[1]).sort(byCodepoint);

    await t.test('the index and the corpus name the same files', (sub) => {
      // Only over a COMPLETE corpus. A partial one holds a subset of the affected files while the
      // index — which a slice does not rewrite — still lists all of them, and that is a missing
      // document, not a missing disclosure.
      const missing = missingDocuments(ed);
      if (missing.length) {
        return sub.skip(`corpus/${ed} is partial — ${missing.join(', ')} not built; the index describes the whole edition`);
      }
      assert.deepEqual(carrying, listed,
        `${ed}: the files carrying "${TOKEN}" and the files corpus/${ed}/INDEX.md lists as retention sites `
        + 'must be the same set. A file in the index and not in the corpus means the note stopped being '
        + 'written; a file in the corpus and not in the index means the index stopped listing it. Either '
        + 'way the disclosure an agent can reach no longer matches the one the build measured.');
      for (const f of listed) {
        assert.ok(fs.existsSync(path.join('corpus', ed, ...f.split('/'))), `${ed}: the index lists ${f}, which does not exist`);
      }
    });

    // NCC 2022 is read from dual-state editorial packages and NCC 2025 is not, so 2022 has
    // retentions (126 sites in 27 source files, measured) and 2025 has none. Both halves are
    // ASSERTED: an empty set on 2022 would mean the whole chain had gone quiet without a word,
    // and a single token in 2025 would mean an edition with no base view had started disclosing
    // one. If a future NCC 2022 dataset genuinely carries no retentions, this is the test that
    // says so out loud, and the ruling to change is R73's — not this assertion.
    if (missingDocuments(ed).length) return;
    if (ed === '2022') {
      assert.ok(listed.length > 0,
        `corpus/${ed}/INDEX.md lists no base-view retentions. The 2022 packages are dual-state editorial `
