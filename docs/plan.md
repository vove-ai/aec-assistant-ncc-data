...
- Frontmatter key order (fixed): `clause`|`term`, `title`, `citation`, `web_url`, `edition`, `volume`, `jurisdiction`, `supersedes`, `building_classes`, `defined_terms`. Omit keys with no value. (`building_classes` is a spec addendum: the 2025 source carries `building` attributes on clauses; dropping data would violate fail-loud.)
