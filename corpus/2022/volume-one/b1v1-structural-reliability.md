...
citation: NCC 2022 V1 B1V1
web_url: https://ncc.abcb.gov.au/editions/ncc-2022/adopted/volume-one/b-structure/part-b1-structural-provisions#B1V1
edition: "2022"
volume: volume-one
jurisdiction: aus
supersedes: "2019: BV1"
...
**(3)** Compliance with and B1P2 is verified for the design of a structural component for strength where—
...
(b) the capacity reduction factors ϕ_G,ϕ_Q,ϕ_W,... are calculated for target reliability indices for permanent action β_(TG), for imposed action β_(TQ), for wind action β_(TW),… in accordance with the equation: β=ln[((R¯)/(S¯))√((C_S)/(C_R))]/√(ln(C_R.C_S)), where—
  ...
  (ii) C_R=1+V_R^2; C_S=1+V_S^2, where—
    ...
    (E) V_S = coefficient of variation of the appropriate action as given in Table B1V1a; and
...
(c) the annual target reliability indices β_(TG),β_(TQ),β_(TW),...are established as follows:
  ...
  (ii) The target reliability indices β_(TG),β_(TQ),β_(TW),... thus established, must be not less than those given in Table B1V1b minus 0.5.
  (iii) For situations where it is not appropriate to compare with an equivalent Deemed-to-Satisfy product, the target reliability index β must be as given in Table B1V1b.
...
| Wind action (γ_W=1.00) (non-cyclonic) | (W¯/W_N)=0.33 | V_W=0.49 |
